...
const pageTitle = computed(() => category.value?.name || "分类");

async function bootstrap() {
  loading.value = true;
  try {
    const { data } = await api.get(`/public/categories/${route.params.id}/posts`);
...
<template>
  <section class="public-list-page" v-loading="loading">
    <article class="panel-card">
      ...
      <h1>{{ pageTitle }}</h1>
      <p class="muted-text">这个分类下共有 {{ posts.length }} 篇已发布文章。</p>
    </article>
    ...
    <div class="post-grid">
      <article v-for="post in posts" :key="post.id" class="post-card">
        ...
        <div class="post-body">
          <p class="post-meta">{{ post.author_name }} · {{ post.created_at }}</p>
          <h3>{{ post.title }}</h3>
